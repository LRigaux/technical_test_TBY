...
PROMPT_FILE = os.path.join(DATA_DIR, "prompt.txt") # Ajuster si déplacé

# --- LLM ---
# Modèle par défaut (peut être surchargé dans l'UI)
DEFAULT_MODEL_REPO_ID = "mistralai/Mistral-7B-Instruct-v0.1"
# DEFAULT_MODEL_REPO_ID = "google/flan-t5-large" # Alternative
...
COLUMN_MAPPING = {
    'standard_keys': ['product_id', 'vendor', 'product_type', 'body_html', 'image_source', 'title'],
    'possible_names': {
        'product_id': ['product_id', 'id', 'sku'],
        'vendor': ['vendor', 'brand', 'marque'],
        'product_type': ['product_type', 'type', 'category'],
        'body_html': ['body_html', 'description', 'desc', 'html_description', 'content'],
