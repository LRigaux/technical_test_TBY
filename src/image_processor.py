...
try:
    ...
    print("Avertissement: Librairie 'rembg' non trouvée. La suppression de fond sera désactivée.")
...
def process_image(
    ...
    try:
        ...
        try:
            ...
            print(f"[{product_id}] Image ouverte avec Pillow. Format original: {original_format}, Mode: {img.mode}")
        except UnidentifiedImageError:
            print(f"[{product_id}] Erreur: Impossible d'identifier le format de l'image depuis l'URL.")
        ...
        if remove_bg_option and REM_BG_AVAILABLE:
            ...
            try:
                ...
                print(f"[{product_id}] Suppression du fond réussie. Nouveau mode: {img.mode}")
                processed = True
            except Exception as e:
                print(f"[{product_id}] Avertissement: Erreur lors de la suppression du fond avec rembg: {e}. On continue sans.")
        ...
        if resize_option:
            ...
            try:
                ...
                print(f"[{product_id}] Redimensionnement réussi. Nouvelle taille: {img.size}")
                processed = True
            except Exception as e:
        ...
        if processed:
            ...
            # Sinon, utiliser le format original si connu, sinon JPEG par défaut
                ...
                save_format = original_format if original_format in ['JPEG', 'PNG', 'GIF'] else 'JPEG'
            ...
            try:
                ...
                if save_format == 'JPEG':
                    ...
                    if img.mode == 'RGBA':
                         print(f"[{product_id}] Conversion RGBA -> RGB pour sauvegarde JPEG...")
                         img = img.convert('RGB')
                elif save_format == 'PNG':
                    save_options['optimize'] = True

                print(f"[{product_id}] Sauvegarde de l'image traitée vers: {processed_image_path} (Format: {save_format})")
                img.save(processed_image_path, format=save_format, **save_options)
                print(f"[{product_id}] Sauvegarde réussie.")

            except Exception as e:
                print(f"[{product_id}] Erreur lors de la sauvegarde de l'image traitée: {e}")
        ...
        print(f"[{product_id}] Erreur réseau lors du téléchargement de l'image {image_url}: {e}")
        return None
        ...
        print(f"[{product_id}] Erreur inattendue lors du traitement de l'image {image_url}: {e}")
        return None